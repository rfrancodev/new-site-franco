...
3. **Arquitetura Resiliente Sem Preflight e Tolerante a CORS (À Prova de Falhas):**
   * *O Problema:* Quando o navegador faz uma requisição `POST` com cabeçalho `Content-Type: application/json` para um domínio externo, as políticas de segurança do navegador exigem o disparo automático de uma requisição preliminar de validação chamada **CORS Preflight (`OPTIONS`)**. Os gateways de proxy e firewalls da nuvem (como os do Google Cloud Run ou Cloudflare) frequentemente filtram ou barram esse método `OPTIONS` por segurança, impedindo que a requisição real `POST` chegue ao backend e resultando no erro do formulário (que aciona a contingência de WhatsApp).
   * *A Solução Inteligente:* 
     * **Remoção do Preflight (`x-www-form-urlencoded`):** Alteramos o formato de envio do frontend para `application/x-www-form-urlencoded` utilizando a API nativa `URLSearchParams`. Essa codificação é considerada uma **"Simple Request"** (Requisição Simples) de acordo com a especificação W3C. Isso faz com que o navegador dispare o `POST` diretamente **sem emitir a chamada de validação `OPTIONS`**, contornando instantaneamente qualquer barreira de gateway e garantindo que o seu backend receba os dados e dispare a automação no n8n de forma imediata!
     * **Decodificação Resiliente de Resposta:** Implementamos uma interceptação inteligente de exceções na leitura do JSON de resposta. Caso o navegador barre a leitura da resposta por restrições locais de privacidade, se o servidor retornou status `HTTP 200 OK`, o frontend interpreta como **Sucesso absoluto**, exibindo a tela de confirmação em vez de lançar falsos negativos e acionar o redirecionamento.
     * **Fallback Automático de Produção:** Se a variável `VITE_API_URL` não for encontrada (por atraso de deploy no Cloudflare), o frontend detecta o ambiente externo em tempo de execução e aponta **automaticamente** para o seu endpoint estável do Cloud Run (`https://ais-pre-dwwk7tdp3w422qgquzvkxx-6392973582.us-west2.run.app`).
